...
class Model:
    ...
    def get_white_lst(self):
        '''获取处理完的白名单'''
        logging.info('start get_white_lst...')
        with open('new_white_list') as fb:
            for line in fb:
                line = line.strip()
                self.white_lst.append(line)
        logging.info('finished get_white_lst...')

    def get_movie_id(self):
        '''获取电影的id和imdb的id'''
        logging.info('start get_movie_id...')
        ...
        logging.info('finished get_movie_id...')
    ...
    def run(self):
        ...
        for movie_id, imdb_id in self.movie_dct.items():
            ...
            if response is None:
                logging.info(f'process movie {self.cur_movie_id} return None')
